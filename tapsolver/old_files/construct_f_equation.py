...
def construct_f_equation(TAPobject_data: TAPobject):
	...
	Returns:
		...
		is subsequently evaluated in the f or inverse problem scripts. 
	...
	def make_g(elementary_process,direction,scale_magnitude):
		...
		if direction == 'f': # standard_parameters['Av']*
			return "((standard_parameters['kbt']*TAPobject_data.species.temperature*(1/TAPobject_data.parameter_scale**("+str(scale_magnitude)+")))/(standard_parameters['h']))*exp(-TAPobject_data.mechanism.processes["+str(elementary_process)+"].f.Ga/(standard_parameters['kbt']*TAPobject_data.species.temperature))"
		else:
			return "((standard_parameters['kbt']*TAPobject_data.species.temperature*(1/TAPobject_data.parameter_scale**("+str(scale_magnitude)+")))/standard_parameters['h'])*exp(-(TAPobject_data.mechanism.processes["+str(elementary_process)+"].f.Ga - TAPobject_data.mechanism.processes["+str(elementary_process)+"].f.dG)/(standard_parameters['kbt']*TAPobject_data.species.temperature))"
	...
	def make_arr(elementary_process,direction):
		...
		if direction == 'f':
			return "TAPobject_data.mechanism.processes["+str(elementary_process)+"].f.Ao*exp(-TAPobject_data.mechanism.processes["+str(elementary_process)+"].f.Ea/(standard_parameters['Rgas']*TAPobject_data.species.temperature))"
		else:
			return "TAPobject_data.mechanism.processes["+str(elementary_process)+"].b.Ao*exp(-TAPobject_data.mechanism.processes["+str(elementary_process)+"].b.Ea/(standard_parameters['Rgas']*TAPobject_data.species.temperature))"

	def make_constant(elementary_process,direction):
		"""Add rate constant for the elementary process specified"""
		if direction == 'f':
			return "TAPobject_data.mechanism.processes["+str(elementary_process)+"].f.k"
		else:
			return "TAPobject_data.mechanism.processes["+str(elementary_process)+"].b.k"

	def make_link(elementary_process):
		"""Add rate constant for the elementary process specified"""
		return "TAPobject_data.mechanism.kinetic_links["+str(elementary_process)+"]"

	# Read through the stoichiometric matrix (i.e. matrix) and define the associated system of odes for the mechanism
