...
def forward_problem(pulse_time, pulse_number, TAPobject_data_original: TAPobject):
    ...
    if type(TAPobject_data.reactor_species.temperature) == dict:
        for j_temp_number, j_temps in enumerate(TAPobject_data.reactor_species.temperature):
            TAPobject_data.reactor_species.temperature[j_temp_number] = Constant(
                TAPobject_data.reactor_species.temperature[j_temp_number])
    else:
        TAPobject_data.reactor_species.temperature = Constant(TAPobject_data.reactor_species.temperature)

    TAPobject_data.reactor_species.reference_temperature = Constant(
        TAPobject_data.reactor_species.reference_temperature)
    ...
    for k in TAPobject_data.reactor_species.gasses:
        ...
        TAPobject_data.reactor_species.gasses[k].intensity = Constant(
            TAPobject_data.reactor_species.gasses[k].intensity)
        total_species += 1
    for k in TAPobject_data.reactor_species.adspecies:
        species_order_dictionary[k] = total_species
        TAPobject_data.reactor_species.adspecies[k].concentration = Constant(
            TAPobject_data.reactor_species.adspecies[k].concentration)
        total_species += 1
    for k in TAPobject_data.reactor_species.inert_gasses:
        species_order_dictionary[k] = total_species
        TAPobject_data.reactor_species.inert_gasses[k].mass = Constant(
            TAPobject_data.reactor_species.inert_gasses[k].mass)
        TAPobject_data.reactor_species.inert_gasses[k].delay = Constant(
            TAPobject_data.reactor_species.inert_gasses[k].delay)
        TAPobject_data.reactor_species.inert_gasses[k].intensity = Constant(
            TAPobject_data.reactor_species.inert_gasses[k].intensity)
        total_species += 1
    ...
    if TAPobject_data.mechanism.reactants != []:
        for k, z in enumerate(TAPobject_data.mechanism.rate_array):
            if TAPobject_data.mechanism.elementary_processes[k].forward.use == 'G':
                TAPobject_data.mechanism.elementary_processes[k].forward.Ga = Constant(
                    TAPobject_data.mechanism.elementary_processes[k].forward.Ga)
                TAPobject_data.mechanism.elementary_processes[k].forward.dG = Constant(
                    TAPobject_data.mechanism.elementary_processes[k].forward.dG)
            elif TAPobject_data.mechanism.elementary_processes[k].forward.use == 'E':
                TAPobject_data.mechanism.elementary_processes[k].forward.Ao = Constant(
                    TAPobject_data.mechanism.elementary_processes[k].forward.Ao)
                TAPobject_data.mechanism.elementary_processes[k].forward.Ea = Constant(
                    TAPobject_data.mechanism.elementary_processes[k].forward.Ea)
            elif TAPobject_data.mechanism.elementary_processes[k].forward.use == 'k':
                TAPobject_data.mechanism.elementary_processes[k].forward.k = Constant(
                    TAPobject_data.mechanism.elementary_processes[k].forward.k)
            ...
            if TAPobject_data.mechanism.elementary_processes[k].backward.use == 'E':
                ...
                TAPobject_data.mechanism.elementary_processes[k].backward.Ao = Constant(
                    TAPobject_data.mechanism.elementary_processes[k].backward.Ao)
                TAPobject_data.mechanism.elementary_processes[k].backward.Ea = Constant(
                    TAPobject_data.mechanism.elementary_processes[k].backward.Ea)
            elif TAPobject_data.mechanism.elementary_processes[k].backward.use == 'k':
                try:
                    TAPobject_data.mechanism.elementary_processes[k].backward.k = Constant(
                        TAPobject_data.mechanism.elementary_processes[k].backward.k)
                except:
                    pass
    ...
    Mesh22 = round(((1 - cat_location) + 0.5 * TAPobject_data.reactor.length_fractions[
        1]) * TAPobject_data.mesh) / TAPobject_data.mesh
    Mesh222 = round(
        ((cat_location) + 0.5 * TAPobject_data.reactor.length_fractions[1]) * TAPobject_data.mesh) / TAPobject_data.mesh
    ...
    Mesh12 = round(((1 - cat_location) - 0.5 * TAPobject_data.reactor.length_fractions[
        1]) * TAPobject_data.mesh) / TAPobject_data.mesh
    Mesh122 = round(
        ((cat_location) - 0.5 * TAPobject_data.reactor.length_fractions[1]) * TAPobject_data.mesh) / TAPobject_data.mesh
    ...
    NumCatCells = meshCells * 2 ** (TAPobject_data.catalyst_mesh_density)
    dx_cat = TAPobject_data.reactor.zone_lengths[1] / NumCatCells
    NumInertCells = int((totalNumCells - NumCatCells) / 2)

    P1 = FiniteElement('CG', mesh.ufl_cell(), 1)

    elements = []
    for k in range(0, len(TAPobject_data.reactor_species.gasses) + len(TAPobject_data.reactor_species.adspecies) + len(
            TAPobject_data.reactor_species.inert_gasses)):
        elements.append(P1)
    ...
    for knum, k in enumerate(list(TAPobject_data.reactor_species.inert_gasses.keys())):
        v_d['v_' + k] = tempA[
            len(TAPobject_data.reactor_species.gasses) + len(TAPobject_data.reactor_species.adspecies) + knum]
        u_d['u_' + k] = tempB[
            len(TAPobject_data.reactor_species.gasses) + len(TAPobject_data.reactor_species.adspecies) + knum]
        u_nd['u_n' + k] = tempC[
            len(TAPobject_data.reactor_species.gasses) + len(TAPobject_data.reactor_species.adspecies) + knum]
    ...
    for k in range(0, len(TAPobject_data.reactor_species.inert_gasses)):
        bcs.append(DirichletBC(
            V.sub(len(TAPobject_data.reactor_species.gasses) + len(TAPobject_data.reactor_species.adspecies) + k),
            Constant(0), boundary_R))
    ...
    if variational_solver == 'constrained':
        snes_solver_parameters = {"nonlinear_solver": "snes",
                                  "snes_solver": {"linear_solver": "lu", "line_search": 'basic',
                                                  "maximum_iterations": 10, "report": False,
    ...
    if TAPobject_data.store_cat_flux == True:
        cat_flux_in = {}
        cat_flux_out = {}
        cat_flux_in['time'] = {}
        cat_flux_out['time'] = {}
    ...
    for k_pulse in range(0, pulse_number):
        ...
        if TAPobject_data.store_cat_flux == True:
            cat_flux_in['time'][k_pulse] = []
            cat_flux_out['time'][k_pulse] = []
            for j in TAPobject_data.reactor_species.gasses:
                cat_flux_in[j][k_pulse] = []
                cat_flux_out[j][k_pulse] = []
            for j in TAPobject_data.reactor_species.inert_gasses:
                cat_flux_in[j][k_pulse] = []
                cat_flux_out[j][k_pulse] = []

        all_species = len(TAPobject_data.reactor_species.gasses) + len(
            TAPobject_data.reactor_species.inert_gasses) + len(TAPobject_data.reactor_species.adspecies)
        time_step = 0
        ...
        while t <= pulse_time:
            ...
            if TAPobject_data.store_flux_data == True:
                for knum, k in enumerate(TAPobject_data.reactor_species.gasses):
                    synthetic_data[k][k_pulse].append(
                        2 * (float(TAPobject_data.reactor_species.gasses[k].inert_diffusion) / (float(dx_r))) * (
                                    float(TAPobject_data.reactor.reactor_radius) ** 2) * 3.14159 * (
                            float(u_n.vector().get_local()[(all_species) + knum])))
                for knum, k in enumerate(TAPobject_data.reactor_species.inert_gasses):
                    synthetic_data[k][k_pulse].append(
                        2 * (float(TAPobject_data.reactor_species.inert_gasses[k].inert_diffusion) / (float(dx_r))) * (
                                    float(TAPobject_data.reactor.reactor_radius) ** 2) * 3.14159 * (float(
                            u_n.vector().get_local()[all_species + len(TAPobject_data.reactor_species.gasses) + len(
                                TAPobject_data.reactor_species.adspecies) + knum])))

            thin_data['time'][k_pulse].append(round(t, 6))
            if TAPobject_data.store_thin_data == True:
                for knum, k in enumerate(TAPobject_data.reactor_species.gasses):
                    thin_data[k][k_pulse].append(float(u_n.vector().get_local()[(catalyst_center_cell * (
                                len(TAPobject_data.reactor_species.gasses) + len(
                            TAPobject_data.reactor_species.adspecies) + len(
                            TAPobject_data.reactor_species.inert_gasses))) + all_species + knum]))
                for knum, k in enumerate(TAPobject_data.reactor_species.adspecies):
                    thin_data[k][k_pulse].append(float(u_n.vector().get_local()[(catalyst_center_cell * (
                                len(TAPobject_data.reactor_species.gasses) + len(
                            TAPobject_data.reactor_species.adspecies) + len(
                            TAPobject_data.reactor_species.inert_gasses))) + all_species + len(
                        TAPobject_data.reactor_species.gasses) + knum]))
                for knum, k in enumerate(TAPobject_data.reactor_species.inert_gasses):
                    thin_data[k][k_pulse].append(float(u_n.vector().get_local()[(catalyst_center_cell * (
                                len(TAPobject_data.reactor_species.gasses) + len(
                            TAPobject_data.reactor_species.adspecies) + len(
                            TAPobject_data.reactor_species.inert_gasses))) + all_species + len(
                        TAPobject_data.reactor_species.gasses) + len(TAPobject_data.reactor_species.adspecies) + knum]))

            cat_flux_in['time'][k_pulse].append(round(t, 6))
            cat_flux_out['time'][k_pulse].append(round(t, 6))
            if TAPobject_data.store_cat_flux == True:
                for knum, k in enumerate(TAPobject_data.reactor_species.gasses):
                    # cat_flux_in[k][k_pulse].append(float(u_n.vector().get_local()[(bottom*(len(TAPobject_data.reactor_species.gasses) + len(TAPobject_data.reactor_species.adspecies) + len(TAPobject_data.reactor_species.inert_gasses)))+all_species+knum]))
                    # cat_flux_in[k][k_pulse].append(float(TAPobject_data.reactor_species.gasses[k].inert_diffusion)*(float(u_n.vector().get_local()[((bottom+1)*(len(TAPobject_data.reactor_species.gasses) + len(TAPobject_data.reactor_species.adspecies) + len(TAPobject_data.reactor_species.inert_gasses)))+all_species+knum]) - float(u_n.vector().get_local()[(bottom*(len(TAPobject_data.reactor_species.gasses) + len(TAPobject_data.reactor_species.adspecies) + len(TAPobject_data.reactor_species.inert_gasses)))+all_species+knum]))/float(dx_r))
                    # cat_flux_out[k][k_pulse].append(float(u_n.vector().get_local()[(top*(len(TAPobject_data.reactor_species.gasses) + len(TAPobject_data.reactor_species.adspecies) + len(TAPobject_data.reactor_species.inert_gasses)))+all_species+knum]))
                    # cat_flux_out[k][k_pulse].append(float(TAPobject_data.reactor_species.gasses[k].catalyst_diffusion)*(float(u_n.vector().get_local()[(top*(len(TAPobject_data.reactor_species.gasses) + len(TAPobject_data.reactor_species.adspecies) + len(TAPobject_data.reactor_species.inert_gasses)))+all_species+knum]) - float(u_n.vector().get_local()[((top-1)*(len(TAPobject_data.reactor_species.gasses) + len(TAPobject_data.reactor_species.adspecies) + len(TAPobject_data.reactor_species.inert_gasses)))+all_species+knum]))/float(dx_r))
                    cat_flux_in[k][k_pulse].append(float(TAPobject_data.reactor_species.gasses[k].inert_diffusion) *
                                                   (float(u_n.vector().get_local()[
                                                              (-NumInertCells) * all_species + knum]) -
                                                    float(u_n.vector().get_local()[
                                                              (-NumInertCells - 1) * all_species + knum])) /
                                                   (dx_r / 2 + dx_cat / 2))
                    cat_flux_out[k][k_pulse].append(float(TAPobject_data.reactor_species.gasses[k].inert_diffusion) *
                                                    (float(u_n.vector().get_local()[
                                                               (-NumInertCells - NumCatCells - 1) * all_species + knum]) -
                                                     float(u_n.vector().get_local()[
                                                               (-NumInertCells - NumCatCells - 1 - 1) * all_species + knum])) /
                                                    (dx_r / 2 + dx_cat / 2))
                for knum, k in enumerate(TAPobject_data.reactor_species.inert_gasses):
                    # cat_flux_in[k][k_pulse].append(float(u_n.vector().get_local()[(bottom*(len(TAPobject_data.reactor_species.gasses) + len(TAPobject_data.reactor_species.adspecies) + len(TAPobject_data.reactor_species.inert_gasses)))+all_species+len(TAPobject_data.reactor_species.gasses)+len(TAPobject_data.reactor_species.adspecies)+knum]))
                    # cat_flux_in[k][k_pulse].append(float(TAPobject_data.reactor_species.gasses[k].inert_diffusion) * (float(u_n.vector().get_local()[((bottom + 1) * (len(TAPobject_data.reactor_species.gasses) + len(TAPobject_data.reactor_species.adspecies) + len(TAPobject_data.reactor_species.inert_gasses))) + all_species + len(TAPobject_data.reactor_species.gasses) + len(TAPobject_data.reactor_species.adspecies) + knum]) - float(u_n.vector().get_local()[(bottom * (len(TAPobject_data.reactor_species.gasses) + len(TAPobject_data.reactor_species.adspecies) + len(TAPobject_data.reactor_species.inert_gasses))) + all_species + len(TAPobject_data.reactor_species.gasses) + len(TAPobject_data.reactor_species.adspecies) + knum])) / float(dx_r))
                    # cat_flux_out[k][k_pulse].append(float(u_n.vector().get_local()[(top*(len(TAPobject_data.reactor_species.gasses) + len(TAPobject_data.reactor_species.adspecies) + len(TAPobject_data.reactor_species.inert_gasses)))+all_species+len(TAPobject_data.reactor_species.gasses)+len(TAPobject_data.reactor_species.adspecies)+knum]))
                    # cat_flux_out[k][k_pulse].append(float(TAPobject_data.reactor_species.gasses[k].catalyst_diffusion) * (float(u_n.vector().get_local()[(top * (len(TAPobject_data.reactor_species.gasses) + len(TAPobject_data.reactor_species.adspecies) + len(TAPobject_data.reactor_species.inert_gasses))) + all_species + len(TAPobject_data.reactor_species.gasses) + len(TAPobject_data.reactor_species.adspecies) + knum]) - float(u_n.vector().get_local()[((top - 1) * (len(TAPobject_data.reactor_species.gasses) + len(TAPobject_data.reactor_species.adspecies) + len(TAPobject_data.reactor_species.inert_gasses))) + all_species + len(TAPobject_data.reactor_species.gasses) + len(TAPobject_data.reactor_species.adspecies) + knum])) / float(dx_r))
                    cat_flux_in[k][k_pulse].append(float(TAPobject_data.reactor_species.gasses[k].inert_diffusion) *
                                                   (float(u_n.vector().get_local()[
                                                              (-NumInertCells) * all_species + len(TAPobject_data.reactor_species.gasses) + len(TAPobject_data.reactor_species.adspecies) + knum]) -
                                                    float(u_n.vector().get_local()[
                                                              (-NumInertCells - 1) * all_species + len(TAPobject_data.reactor_species.gasses) + len(TAPobject_data.reactor_species.adspecies) + knum])) /
                                                   (dx_r / 2 + dx_cat / 2))
                    cat_flux_out[k][k_pulse].append(float(TAPobject_data.reactor_species.gasses[k].inert_diffusion) *
                                                    (float(u_n.vector().get_local()[
                                                               (-NumInertCells - NumCatCells - 1) * all_species + len(TAPobject_data.reactor_species.gasses) + len(TAPobject_data.reactor_species.adspecies) + knum]) -
                                                     float(u_n.vector().get_local()[
                                                               (-NumInertCells - NumCatCells - 1 - 1) * all_species + len(TAPobject_data.reactor_species.gasses) + len(TAPobject_data.reactor_species.adspecies) + knum])) /
                                                    (dx_r / 2 + dx_cat / 2))
            ...
            if TAPobject_data.optimize == True:
                    ...
                    if round(t, 6) in output_data['time'][0]:
                        ...
                        try:
                            if k_fitting in TAPobject_data.reactor_species.gasses:
                                jfunc_2 += assemble(inner(u_n[species_order_dictionary[k_fitting]] * (2 * (
                                            TAPobject_data.reactor_species.gasses[k_fitting].inert_diffusion / (
                                        dx_r)) * (TAPobject_data.reactor.reactor_radius ** 2) * 3.14159) - w3,
                                                          u_n[species_order_dictionary[k_fitting]] * (2 * (
                                                                      TAPobject_data.reactor_species.gasses[
                                                                          k_fitting].inert_diffusion / (dx_r)) * (
                                                                                                                  TAPobject_data.reactor.reactor_radius ** 2) * 3.14159) - w3) * dP(
                                    1)) / ((TAPobject_data.reactor_species.gasses[k_fitting].sigma) ** 2)
                            elif k_fitting in TAPobject_data.reactor_species.inert_gasses:
                                jfunc_2 += assemble(inner(u_n[species_order_dictionary[k_fitting]] * (2 * (
                                            TAPobject_data.reactor_species.inert_gasses[k_fitting].inert_diffusion / (
                                        dx_r)) * (TAPobject_data.reactor.reactor_radius ** 2) * 3.14159) - w3,
                                                          u_n[species_order_dictionary[k_fitting]] * (2 * (
                                                                      TAPobject_data.reactor_species.inert_gasses[
                                                                          k_fitting].inert_diffusion / (dx_r)) * (
                                                                                                                  TAPobject_data.reactor.reactor_radius ** 2) * 3.14159) - w3) * dP(
                                    1)) / ((TAPobject_data.reactor_species.inert_gasses[k_fitting].sigma) ** 2)
                ...
                if TAPobject_data.thermodynamic_constraints == True and t == 0:
                    ...
                    for kip in thermoReactions.keys():
                        w_temp[kip] = Expression(str(reactor_kinetics_input['thermo values'][kip]),
                                                 degree=0)  # deltaG = sum(-R*T*ln(kf/kb))
                        ...
                        for jnum, jval in enumerate(thermoReactions[kip]):
                            ...
                            if jnum == 0:
                                tempFunc[kip] = thermoStoich[kip][jnum] * (
                                            -0.008314 * reac_input['Reactor Temperature']) * ln(
                                    r_const["kf" + str(jval - 1)] / r_const["kb" + str(jval - 1)])
                            else:
                                tempFunc[kip] += thermoStoich[kip][jnum] * (
                                            -0.008314 * reac_input['Reactor Temperature']) * ln(
                                    r_const["kf" + str(jval - 1)] / r_const["kb" + str(jval - 1)])
            ...
            if TAPobject_data.tangent_linear_sensitivity == True:
                for knum, k in enumerate(TAPobject_data.reactor_species.gasses):
                    new_val = 2 * (TAPobject_data.reactor_species.gasses[k].inert_diffusion / (dx_r)) * (
                                TAPobject_data.reactor.reactor_radius ** 2) * 3.14159
    ...
    if TAPobject_data.optimize == True:
        ...
        rf_2 = ReducedFunctional(jfunc_2, controls, tape=tape2,
                                 derivative_cb_post=derivCB)  # ,derivative_cb_post=derivCB,hessian_cb_post=hessCB)
            ...
            if k != 'time':
                for j in synthetic_data[k]:
                    for z in range(0, len(synthetic_data[k][j])):
                        try:
                            synthetic_data[k][j][z] += np.random.normal(0, 1) * TAPobject_data.reactor_species.gasses[
                                k].noise  # +beta_2*np.cos(w_2*(k*dt))

                        except:
                            try:
                                synthetic_data[k][j][z] += np.random.normal(0, 1) * \
                                                           TAPobject_data.reactor_species.inert_gasses[
                                                               k].noise  # +beta_2*np.cos(w_2*(k*dt))
                            except:
                                pass
                ...
                for j in thin_data[k]:
                    for z in range(0, len(thin_data[k][j])):
                        if k in TAPobject_data.reactor_species.gasses:
                            try:
                                thin_data[k][j][z] += np.random.normal(0, 1) * TAPobject_data.reactor_species.gasses[
                                    k].noise  # +beta_2*np.cos(w_2*(k*dt))
                            except:
                                pass
                        if k in TAPobject_data.reactor_species.adspecies:
                            try:
                                thin_data[k][j][z] += np.random.normal(0, 1) * TAPobject_data.reactor_species.adspecies[
                                    k].noise  # +beta_2*np.cos(w_2*(k*dt))
                            except:
                                pass
                        if k in TAPobject_data.reactor_species.inert_gasses:
                            try:
                                thin_data[k][j][z] += np.random.normal(0, 1) * \
                                                      TAPobject_data.reactor_species.inert_gasses[
                                                          k].noise  # +beta_2*np.cos(w_2*(k*dt))
                            except:
                                pass
    ...
    if TAPobject_data.store_cat_flux == True:
        save_object(cat_flux_in, './' + TAPobject_data.output_name + '/TAP_cat_in.json')
        save_object(cat_flux_out, './' + TAPobject_data.output_name + '/TAP_cat_out.json')
