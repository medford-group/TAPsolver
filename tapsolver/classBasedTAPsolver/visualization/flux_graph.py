...
def flux_graph(TAPobject_data: TAPobject):
	...
	for jnum,j in enumerate(TAPobject_data.reactor_species.gasses):
		for k in synthetic_data[j]:
			if k == 0:
				plt.plot(synthetic_data['time'][0], synthetic_data[j][0],label=j,color=colors[jnum])
			else:
				plt.plot(synthetic_data['time'][0], synthetic_data[j][0],color=colors[jnum])

	for jnum,j in enumerate(TAPobject_data.reactor_species.inert_gasses):
		for k in synthetic_data[j]:
			if k == 0:
				plt.plot(synthetic_data['time'][0], synthetic_data[j][0],label=j,color=colors[jnum+len(TAPobject_data.reactor_species.gasses.keys())])		
			else:
				plt.plot(synthetic_data['time'][0], synthetic_data[j][0],color=colors[jnum+len(TAPobject_data.reactor_species.gasses.keys())])
	
	for jnum,j in enumerate(TAPobject_data.reactor_species.gasses):
		for k in experimental_data[j]:
			if k == 0:
				plt.scatter(experimental_data['time'][0], experimental_data[j][0],label=j+'_exp',s=0.3,color=colors[jnum])
			else:
				plt.scatter(experimental_data['time'][0], experimental_data[j][0],s=0.3,color=colors[jnum])
	
	for jnum,j in enumerate(TAPobject_data.reactor_species.inert_gasses):
		for k in experimental_data[j]:
			if k == 0:
				plt.scatter(experimental_data['time'][0], experimental_data[j][0],label=j+'_exp',s=0.3,color=colors[jnum+len(TAPobject_data.reactor_species.gasses.keys())])
			else:
				plt.scatter(experimental_data['time'][0], experimental_data[j][0],s=0.3,color=colors[jnum+len(TAPobject_data.reactor_species.gasses.keys())])
