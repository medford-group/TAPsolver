...
def solver_iteration(time_step,method,solver,dec_tim,inc_tim):
	try:
		...
		if time_step < 1e-5:
			...
			sys.exit()
		time_step=solver_iteration(time_step,solver_method_options,1.5,1.1)
		return time_step
	...
	if reactor == 'tap':
		...
		if elem_list != ['INERT_ONLY']:
			for k in range(0,nec_values):
				bcs.append(DirichletBC(V_sec.sub(k),Constant(1),R_bound))
			bcs.append(DirichletBC(V_sec.sub(all_mol),Constant(1),R_bound))
		else:	
			bcs.append(DirichletBC(V_sec,Constant(0),R_bound))
	
	elif reactor == 't_pfr' or 't_pfr_diff':
		bcs = []
		for k in range(0,reacs_num):
