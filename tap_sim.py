...
bcs = define_boundary_conditions(reactor_type,reactions_test,necessary_values['molecules_in_gas_phase'],V,reactants_num,monitored_gas,reactant_ratio,boundary_L,boundary_R)
...
for k_pulse in range(0,number_of_pulses):
	...
	while t <= Time:
		...
		if t > 0:
			dt = solver_iteration(dt,solver_method_options,solver,1.5,1.1)																											
