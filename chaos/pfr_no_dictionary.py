...
#######################################################		### Remove solving iterations from the output
...
#######################################################		### Reaction and reactor options for PFR
#reactions_test = ['O2 <-> 2O','CO + O -> CO2']
reactions_test = ['A <-> B']
#reactions_test = ['A <-> B','B <-> C','A + B <-> D', 'C + D <-> E']

reactants_num = 1
Inert_pulse_size = (2.5e-8)*(6.022e23)
reactant_ratio = [1,1]
...
#######################################################		### Parameters and definitions in fenics
...
######################		### Define boundaries and subdomains in fenics
...
######################		### Define the velocity profile in the 1d reactor
...
F = ((u_1 - u_n1))*v_1*dx(0)  + dk*Constant((np.sum(r_param)))*dot(w, grad(u_1))*v_1*dx(0) +((u_1 - u_n1))*v_1*dx(1)  + dk*Constant((np.sum(r_param)))*dot(w, grad(u_1))*v_1*dx(1) + ((u_2 - u_n2))*v_2*dx(0)  + dk*Constant((np.sum(r_param)))*dot(w, grad(u_2))*v_2*dx(0) + ((u_2 - u_n2) )*v_2*dx(1)  + dk*Constant((np.sum(r_param)))*dot(w, grad(u_2))*v_2*dx(1) + ((u_3 - u_n3))*v_3*dx(0) + dk*((u_3 - u_n3) )*v_3*dx(1)- dk*(1.0* Kd0*(u_2**1.0)*v_1*dx(1))+dk*(1.0* Ke0*(u_1**1.0)*v_1*dx(1))+dk*(1.0* Kd0*(u_2**1.0)*v_2*dx(1))-dk*(1.0* Ke0*(u_1**1.0)*v_2*dx(1))+ ((u_4 - u_n4))*v_4*dx(0)+ dk * Constant((np.sum(r_param))) * dot(w, grad(u_4))*v_4*dx(0) +((u_4 - u_n4) )*v_4*dx(1) + dk * Constant((np.sum(r_param))) * dot(w, grad(u_4))*v_4*dx(1)
...
##((u_d['u_1'] - u_nd['u_n1']))*v_d['v_1']*dx(0)  + dk*Constant((np.sum(r_param)))*dot(w, grad(u_d['u_1']))*v_d['v_1']*dx(0) + 
##((u_d['u_1'] - u_nd['u_n1']) )*v_d['v_1']*dx(1)  + dk*Constant((np.sum(r_param)))*dot(w, grad(u_d['u_1']))*v_d['v_1']*dx(1) + 

##((u_d['u_2'] - u_nd['u_n2']))*v_d['v_2']*dx(0)  + dk*Constant((np.sum(r_param)))*dot(w, grad(u_d['u_2']))*v_d['v_2']*dx(0) + 
##((u_d['u_2'] - u_nd['u_n2']) )*v_d['v_2']*dx(1)  + dk*Constant((np.sum(r_param)))*dot(w, grad(u_d['u_2']))*v_d['v_2']*dx(1) + 

##((u_d['u_3'] - u_nd['u_n3']))*v_d['v_3']*dx(0) + dk*((u_d['u_3'] - u_nd['u_n3']) )*v_d['v_3']*dx(1)- 

##dk*(1.0* Kd0*(u_d['u_2']**1.0)*v_d['v_1']*dx(1))+
##dk*(1.0* Ke0*(u_d['u_1']**1.0)*v_d['v_1']*dx(1))+ 
##dk*(1.0* Kd0*(u_d['u_2']**1.0)*v_d['v_2']*dx(1))-
##dk*(1.0* Ke0*(u_d['u_1']**1.0)*v_d['v_2']*dx(1))+ 

##((u_d['u_4'] - u_nd['u_n4']))*v_d['v_4']*dx(0)+ dk * Constant((np.sum(r_param))) * dot(w, grad(u_d['u_4']))*v_d['v_4']*dx(0) +
##((u_d['u_4'] - u_nd['u_n4']) )*v_d['v_4']*dx(1) + dk * Constant((np.sum(r_param))) * dot(w, grad(u_d['u_4']))*v_d['v_4']*dx(1)

############################# ADD THE NON-DICTIONARY VERSION HERE ##################################
...
bcs.append(DirichletBC(V.sub(2),Constant(1),boundary_L))
