...
syn_data_folder = './analysis/CO_O2_analysis/CO_O2_exp_folder/flux_data/'#'./analysis/CO_O2_analysis/CO_O2_exp_folder/flux_data/'
#two_syn_data_folder = './analysis/lh_eh_co_analysis/paper_ER_exp_folder/flux_data/'
exp_data_folder = './analysis/CO_O2_analysis/CO_O2_fit_productobj_bounds_folder/fitting/'#'./analysis/CO_O2_analysis/CO_O2_fit_productobj_3rd_folder/fitting/'
num_in_folder = 32
read_data = './analysis/CO_O2_analysis/CO_O2_fit_productobj_bounds_folder/your_file.txt'#'./analysis/lh_eh_co_analysis/isolated_CO_fit5_folder/fitting/your_file.txt'#'./objective_function_efficiency/both_1e-13_3_folder/fitting/your_file.txt'#'./analysis/CO_O2_analysis/CO_O2_fit_productobj_3rd_folder/fitting/your_file.txt'
reactants = ['CO','O2','CO2','Inert']
#reactants = ['A','B']
prop_mech = ['CO + * <-> CO*','O2 + 2* <-> 2O*','CO + O* -> CO2 + *']
#prop_mech = ['O2 + 2* <-> 2O*']
#prop_mech = ['$A$ $Diffusion$','$B$ $Diffusion$']
act_mech = ['CO + * <-> CO*','O2 + 2* <-> 2O*','CO* + O* -> CO2 + 2*']
#act_mech = ['O2 + 2* <-> 2O*']
#act_mech = ['$A$ $Diffusion$','$B$ $Diffusion$']
#act_const = [10,50]
act_const = [0.001,0.0001,1e-7,1,1]
...
def generate_gif(molecules,exp_loc,fit_loc,all_steps,constants,reactions,time_data,actual_mech):
	...
	def tap_plot(step):
		...
		textstr = '\n'.join((textstr,constant_names[4]+': '+'{:0.2e}'.format(constants[step][0])))
