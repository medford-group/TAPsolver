...
def curveFitting(species_list,sim_steps,folder,timeTot,points,objSpecies):
	...
	for k_newNum, k_new in enumerate(species_list):
		if objSpecies[k_newNum] == '1':
			...
			for k in range(65,int(sim_steps),frequency):
				time_step.append(k)
...
def generateGif(molecules,exp_loc,fit_loc,all_steps,constants,reactions,time_data):
	...
	def tap_plot(step):
		...
		labels = ['$CO$','$O_{2}$','$CO_{2}$','$Ar$','$He$']

		for k_names_num,k_names in enumerate(molecules):
			ax.scatter(exp_data[k_names][0], exp_data[k_names][1],alpha=0.3,label=labels[k_names_num])
		
		for k_names in molecules:
			ax.plot(sim_data[k_names][0], sim_data[k_names][1],ls='--')
	
		ax.set_xlabel('Time (s)', fontsize=16)
		ax.set_ylabel('Outlet Flow (nmol/s)', fontsize=16)
