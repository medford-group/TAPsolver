...
output_file_name = "ross_lin"	# "./FILENAME.csv"
theta = 1 						# forward_euler = 0, backward_euler = 1, crank_nicolson = 1/2
solver_method_options = 'None'	#'simple_adaptive' or None 
Time = 4						# Total pulse time
Time_steps = 1000  				#time_step_value	# Number of time steps  
...
reactants_num = 1
Inert_pulse_size = (3e-6)*6.022e23	###Size of inert pulse (# of molecules, not mol)#5e-8 #(5e-9)* 
...
length_reac = 3.6
factor = 5/20
len_inert_1 = length_reac/2 -  0.5*(factor)*length_reac#2.54*(19/20)/2		###Length of the first inert zone (cm)
len_cat = (factor)*length_reac#2.54*(1/20)			###Length of the catalyst zone (cm) 
len_inert_2 =  length_reac/2 -  0.5*(factor)*length_reac#2.54*(19/20)/2		###Length of the second inert zone (cm)
reac_radius = sqrt((1.1e-5)*(100**2)/3.14159)#sqrt((1.8e-5)*(100**2)/3.14159)		###Radius of the reactor

############# Transport properties ##########################################################################

void_inert = 1			###Voidage of the catalyst
void_cat = 1			###Voidage of the catalyst

ref_rate_inert = 43.5319		###Reference 
ref_rate_cat = 43.5319
ref_mass = 423.15
mass_list = np.array((179,179,179))
...
rangesurface_species = [0,0]

#Ke0 = 1000*(100**3)/(6.022e23)
Ke0 = 1/(((length_reac)/mesh_size))#sqrt((100**3)/(6.022e23))
print(Ke0)
Ke1 = 2
reactor_kinetics_input = dict(locals())

time_of_sim, graph_data, legend_label = tap_simulation_function(reactor_kinetics_input)
df0 = pd.read_csv('./ross_lin_folder/Inert.csv',header=None)
df1 = pd.read_csv('./toy_data/linA.txt',sep=' ',header=None)
df2 = pd.read_csv('./ross_lin_folder/A.csv',header=None)
df3 = pd.read_csv('./toy_data/linB.txt',sep=' ',header=None)
df4 = pd.read_csv('./ross_lin_folder/B.csv',header=None)
fig3, ax3 = plt.subplots()
ax3.plot(df0.iloc[:,0],df0.iloc[:,1],label='Inert Fenics',ls = '--')
ax3.plot(df1.iloc[:,0],df1.iloc[:,1],label='A Ross',ls = '--')
ax3.plot(df2.iloc[:,0],df2.iloc[:,1],label='A Fenics',ls = '--')
ax3.plot(df3.iloc[:,0],df3.iloc[:,1],label='B Ross',ls = '--')
ax3.plot(df4.iloc[:,0],df4.iloc[:,1],label='B Fenics',ls = '--')
ax3.legend(title="Data")
plt.show()
